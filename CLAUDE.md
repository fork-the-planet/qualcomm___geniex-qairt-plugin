...
- **HTP version**: The bundled runtime libs in `third-party/` are QAIRT v2.45.0.260326 (`windows`, `android`, `linux-gcc11.2`). The version string lives in `core/include/version.h` (`GENIEX_QAIRT_RUNTIME_VERSION`) — update that macro **and** this sentence when refreshing the bundled binaries. Runtime version is backward compatible with compile version, so models compiled with an older QAIRT (e.g. v2.42) will run fine. When adding a new model, verify its compile version from the `buildId` field in the shard `.json` config files and ensure the `dsp_arch` in `htp_backend_ext_config.json` matches the target `third-party/` folder (e.g. `v73` → `windows`).
