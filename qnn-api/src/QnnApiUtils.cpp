...
bool updateTensorInfo(const Qnn_Tensor_t* tensorsInfoSrc, Qnn_Tensor_t* tensorWrappers, uint32_t tensorsCount) {
    for (size_t tIdx = 0; tIdx < tensorsCount; tIdx++) {
        ...
        qParams.encodingDefinition   = QNN_TENSOR_GET_QUANT_PARAMS(&tensorsInfoSrc[tIdx]).encodingDefinition;
        qParams.quantizationEncoding = QNN_QUANTIZATION_ENCODING_UNDEFINED;
        if (QNN_TENSOR_GET_QUANT_PARAMS(&tensorsInfoSrc[tIdx]).quantizationEncoding ==
            QNN_QUANTIZATION_ENCODING_SCALE_OFFSET) {
            qParams.quantizationEncoding = QNN_TENSOR_GET_QUANT_PARAMS(&tensorsInfoSrc[tIdx]).quantizationEncoding;
            qParams.scaleOffsetEncoding  = QNN_TENSOR_GET_QUANT_PARAMS(&tensorsInfoSrc[tIdx]).scaleOffsetEncoding;
            ...
            qParams.axisScaleOffsetEncoding.numScaleOffsets =
                QNN_TENSOR_GET_QUANT_PARAMS(&tensorsInfoSrc[tIdx]).axisScaleOffsetEncoding.numScaleOffsets;
            if (QNN_TENSOR_GET_QUANT_PARAMS(&tensorsInfoSrc[tIdx]).axisScaleOffsetEncoding.numScaleOffsets > 0) {
    ...
    if (returnStatus) {
        for (size_t tIdx = 0; tIdx < tensorsCount; tIdx++) {
            ...
            if (QNN_TENSOR_GET_RANK(tensorsInfoSrc[tIdx]) > 0) {
                ...
                if (QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(&tensorsInfoSrc[tIdx])) {
                    QNN_TENSOR_SET_IS_DYNAMIC_DIMENSIONS(
                        tensor, (uint8_t*)malloc(QNN_TENSOR_GET_RANK(&tensorsInfoSrc[tIdx]) * sizeof(uint8_t)));
                    if (QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(tensor)) {
                        memcpy(QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(tensor),
                            QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(&tensorsInfoSrc[tIdx]),
                            QNN_TENSOR_GET_RANK(&tensorsInfoSrc[tIdx]) * sizeof(uint8_t));
                    }
                }
            }
            QNN_TENSOR_SET_SPARSE_PARAMS(tensor, QNN_TENSOR_GET_SPARSE_PARAMS(&tensorsInfoSrc[tIdx]));
        }
    }
...
bool updateGraphInfoV1(const QnnSystemContext_GraphInfoV1_t* graphInfoSrc, qnn_wrapper_api::GraphInfo_t* graphInfoDst) {
    if (graphInfoSrc->graphInputs) {
        if (!updateTensorInfo(graphInfoSrc->graphInputs, graphInfoDst->inputTensors, graphInfoSrc->numGraphInputs)) {
            return false;
        }
    }
    if (graphInfoSrc->graphOutputs) {
        if (!updateTensorInfo(graphInfoSrc->graphOutputs, graphInfoDst->outputTensors, graphInfoSrc->numGraphOutputs)) {
            return false;
        }
    }
    return true;
...
bool updateGraphInfoV3(const QnnSystemContext_GraphInfoV3_t* graphInfoSrc, qnn_wrapper_api::GraphInfo_t* graphInfoDst) {
    if (graphInfoSrc->graphInputs) {
        if (!updateTensorInfo(graphInfoSrc->graphInputs, graphInfoDst->inputTensors, graphInfoSrc->numGraphInputs)) {
            return false;
        }
...
bool copyGraphsInfoV1(const QnnSystemContext_GraphInfoV1_t* graphInfoSrc, qnn_wrapper_api::GraphInfo_t* graphInfoDst) {
...
bool updateGraphInfo(const QnnSystemContext_GraphInfo_t* graphsInput, const uint32_t numGraphs,
...
bool copyGraphsInfo(const QnnSystemContext_GraphInfo_t* graphsInput, const uint32_t numGraphs,
    ...
    auto returnStatus = true;
    graphsInfo        = (qnn_wrapper_api::GraphInfo_t**)calloc(numGraphs, sizeof(qnn_wrapper_api::GraphInfo_t*));
    ...
    if (true == returnStatus) {
        for (size_t gIdx = 0; gIdx < numGraphs; gIdx++) {
            ...
            auto graphInfoArr = (qnn_wrapper_api::GraphInfo_t*)malloc(sizeof(qnn_wrapper_api::GraphInfo_t));
...
bool updateMetaDataToGraphsInfo(
    const QnnSystemContext_BinaryInfo_t* binaryInfo, qnn_wrapper_api::GraphInfo_t** graphsInfo, uint32_t& graphsCount) {
    if (nullptr == binaryInfo) {
        QNN_ERROR("binaryInfo is nullptr.");
        return false;
    }
...
bool copyMetadataToGraphsInfo(const QnnSystemContext_BinaryInfo_t* binaryInfo,
    ...
    if (binaryInfo->version == QNN_SYSTEM_CONTEXT_BINARY_INFO_VERSION_1) {
        if (binaryInfo->contextBinaryInfoV1.graphs) {
            if (!copyGraphsInfo(
                    binaryInfo->contextBinaryInfoV1.graphs, binaryInfo->contextBinaryInfoV1.numGraphs, graphsInfo)) {
                QNN_ERROR("Failed while copying graphs Info.");
                return false;
            }
        ...
        if (binaryInfo->contextBinaryInfoV2.graphs) {
            if (!copyGraphsInfo(
                    binaryInfo->contextBinaryInfoV2.graphs, binaryInfo->contextBinaryInfoV2.numGraphs, graphsInfo)) {
                QNN_ERROR("Failed while copying graphs Info.");
                return false;
            }
        ...
        if (binaryInfo->contextBinaryInfoV3.graphs) {
            if (!copyGraphsInfo(
                    binaryInfo->contextBinaryInfoV3.graphs, binaryInfo->contextBinaryInfoV3.numGraphs, graphsInfo)) {
                QNN_ERROR("Failed while copying graphs Info.");
                return false;
            }
