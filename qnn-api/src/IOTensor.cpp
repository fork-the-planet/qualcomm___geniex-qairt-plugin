...
IOTensor::IOTensor(BufferAlloc bufferAllocIn, QNN_INTERFACE_VER_TYPE* qnnInterface)
    : m_bufferAlloc(bufferAllocIn), m_qnnInterface(qnnInterface), m_bufferManager(new ClientBuffer()) {}
...
bool IOTensor::initialize(Qnn_ContextHandle_t contextHandle, uint32_t dataAlignmentSize) {
    if (m_bufferAlloc == BufferAlloc::SHARED_BUFFER) {
        m_bufferManager = std::unique_ptr<IBufferAlloc>(new RpcMem(contextHandle, m_qnnInterface, dataAlignmentSize));
    } else if (m_bufferAlloc == BufferAlloc::DMABUF) {
#ifdef _WIN32
        return false;
#else
        m_bufferManager = std::unique_ptr<IBufferAlloc>(new DmaBufferAllocator(contextHandle, m_qnnInterface));
#endif
    }
...
bool IOTensor::setupTensors(Qnn_Tensor_t** tensors, std::unordered_map<std::string, void*>& tensorNameToTensorPointer,
    uint32_t tensorCount, Qnn_Tensor_t* tensorWrappers, std::unordered_map<std::string, size_t>& tensorsSize,
    Qnn_ContextHandle_t contextHandle, bool skipBufferAllocation) {
...
bool IOTensor::setupInputTensors(Qnn_Tensor_t** inputs,
    std::unordered_map<std::string, void*>& tensorNameToTensorPointer, const qnn_wrapper_api::GraphInfo_t& graphInfo,
    std::unordered_map<std::string, size_t>& inputTensorsSize, Qnn_ContextHandle_t contextHandle,
...
bool IOTensor::setupOutputTensors(Qnn_Tensor_t** outputs,
    std::unordered_map<std::string, void*>& tensorNameToTensorPointer, const qnn_wrapper_api::GraphInfo_t& graphInfo,
    std::unordered_map<std::string, size_t>& outputTensorsSize, Qnn_ContextHandle_t contextHandle,
    bool skipBufferAllocation) {
...
bool IOTensor::setupTensorWithSharedBuffers(Qnn_Tensor_t** tensors,
    std::unordered_map<std::string, void*>& tensorNameToTensorPointer, uint32_t tensorCount,
    Qnn_Tensor_t* tensorWrappers, std::unordered_map<std::string, size_t>& tensorsSize,
    ...
    for (size_t tensorIdx = 0; tensorIdx < tensorCount; tensorIdx++) {
        ...
        if (true == returnStatus) {
            if (sharedTensorMap.find(wrapperTensorName) == sharedTensorMap.end()) {
                ...
                returnStatus =
                    m_bufferManager->useSameMemory(((*tensors) + tensorIdx), sharedTensorMap[wrapperTensorName]);
            }
        }
        if (true != returnStatus) {
            QNN_ERROR("Failure in setupTensors, cleaning up resources");
...
bool IOTensor::mapFusedBufferOffset(qnn_wrapper_api::GraphInfo_t* graph_info, Qnn_ContextHandle_t context_handle,
