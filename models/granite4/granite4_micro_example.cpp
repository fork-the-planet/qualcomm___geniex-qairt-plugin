...
static std::string applyTemplate(const std::string& user_text,
    ...
    std::string prompt;
    if (first_turn) {
        prompt = "<|start_of_role|>system<|end_of_role|>" + system_prompt +
                 "<|end_of_text|>\n";
    }
    prompt += "<|start_of_role|>user<|end_of_role|>" + user_text +
...
int main(int argc, char** argv) {
    ...
    geniex::LLMModel model = geniex::granite4_micro::makeModel();
    try {
