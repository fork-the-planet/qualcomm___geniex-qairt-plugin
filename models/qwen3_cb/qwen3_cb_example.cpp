...
int main(int argc, char** argv) {
    ...
    auto model = geniex::qwen3_cb::makeModel();
    try {
    ...
    while (true) {
        ...
        const int total_display_lines = static_cast<int>(n_sessions)
                                        * (lines_per_session + 1);  // +1 for label line
        ...
        auto textToRows = [&](const std::string& text) -> std::vector<std::string> {
            ...
            while (pos < text.size()) {
                size_t nl = text.find('\n', pos);
                std::string logical_line = (nl == std::string::npos)
                ...
                while (lp < logical_line.size()) {
                    size_t chunk = std::min(static_cast<size_t>(text_width),
                                            logical_line.size() - lp);
        ...
        auto redraw = [&]() {
            ...
            for (size_t i = 0; i < n_sessions; ++i) {
                ...
                for (; printed_lines < lines_per_session; ++printed_lines)
                    std::cout << "\r\033[K\n";
            }
            std::cout << std::flush;
        };
        ...
        try {
            model.generateBatch(scheduler, kv_mgr,
                [&](const std::string& sid, int32_t tok) {
                    size_t idx = 0;
        ...
        if (args.verbose) {
            ...
            std::cout << "\n\033[1;36m=== Performance ===\033[0m\n"
                      ...
                      << elapsed_ms << " ms\n"
                      << "Throughput     : " << std::setprecision(2) << tps
                      << " tokens/s\n"
                      << "===================\n";
        }
        std::cout << "\n";
