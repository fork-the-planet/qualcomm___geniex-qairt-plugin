...
std::vector<float> Qwen25VLVisionEncoder::encode(const PixelData& pixel_data) {
    ...
    const auto window_index =
        qwen_vit::computeWindowIndex(kGridT, kGridH, kGridW, kSpatialMergeSize, kVitWindowSize, kPatchSize);
    const auto reverse_index = qwen_vit::reverseWindowIndex(window_index);
    const auto cu_window =
        qwen_vit::computeCuWindowSeqlens(kGridT, kGridH, kGridW, kSpatialMergeSize, kVitWindowSize, kPatchSize);

    const auto inv_freq               = qwen_vit::makeInvFreq(kVitRopeDim, kVitRopeTheta);
    auto [rope_cos_nat, rope_sin_nat] = qwen_vit::computePatchRoPE(kGridT, kGridH, kGridW, kSpatialMergeSize, inv_freq);
    ...
    const auto      full_mask = qwen_vit::buildBlockAttentionMask(N, {0, static_cast<int64_t>(N)}, kAllowed, kBlocked);
    ...
    for (size_t img = 0; img < n_images; ++img) {
        ...
        for (size_t i = 0; i < reverse_index.size(); ++i) {
            ...
            std::memcpy(
                out_slot + i * kHiddenSize, tokens_reordered.data() + src * kHiddenSize, kHiddenSize * sizeof(float));
        }
    }
...
std::unique_ptr<Qwen25VLModel> makeModel(const QnnRuntimeConfig& runtime_cfg, const VLMConfig& config) {
    ...
    model->setMRoPEProvider(std::make_unique<MRoPEInputProvider>(mRoPESection(), kRopeTheta, MRoPEInterleaving::BLOCK));
