...
class GENIEX_API RoPEEncoder : public PositionEncoder {
   public:
    RoPEEncoder(size_t head_dim, float theta) : rope_(head_dim, theta) {}
...
GENIEX_API std::unique_ptr<PositionEncoder> makePositionEncoder(
    PositionEncodingType type, size_t head_dim, float rope_theta);
