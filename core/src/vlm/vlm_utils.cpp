...
MRoPEPositions computeMRoPEPositions(const std::vector<int32_t>& input_ids, const std::vector<ImageGrid>& image_grids,
    const std::vector<AudioSegmentInfo>& audio_segments, int spatial_merge_size, int32_t vision_start_token_id,
    int32_t image_token_id, int32_t audio_start_token_id, int32_t audio_token_id) {
    ...
    while (i < seq_len) {
        ...
        if (tok == vision_start_token_id && image_idx < static_cast<int>(image_grids.size())) {
        ...
        } else if (tok == audio_start_token_id && audio_idx < static_cast<int>(audio_segments.size())) {
            temporal[i] = height[i] = width[i] = st++;
            ++i;
    ...
    const int32_t max_pos = (seq_len > 0) ? *std::max_element(temporal, temporal + seq_len) : 0;
