...
std::vector<float> makeInvFreq(int dim, float theta) {
    ...
    for (int i = 0; i < half; ++i) inv[i] = 1.0f / std::pow(theta, float(i * 2) / float(dim));
    return inv;
}

std::vector<int64_t> computeWindowIndex(int T, int H, int W, int spatial_merge_size, int window_size, int patch_size) {
    ...
    for (int t = 0; t < T; ++t)
        for (int h = 0; h < llm_h; ++h)
            for (int w = 0; w < llm_w; ++w)
                idx_pad[t * pH * pW + h * pW + w] = static_cast<int64_t>(t * llm_h * llm_w + h * llm_w + w);
...
std::pair<std::vector<float>, std::vector<float>> computeSpatialCosSin(int T, int H, int W, int spatial_merge_size,
    const std::vector<float>& inv_freq, const std::vector<int64_t>& window_index) {
    const int llm_h    = H / spatial_merge_size;
    const int llm_w    = W / spatial_merge_size;
...
std::vector<float> windowReorder(const std::vector<float>& hidden, size_t n_groups, size_t sm_unit, size_t embed_dim,
    ...
    for (size_t i = 0; i < window_index.size(); ++i) {
        ...
        std::memcpy(out.data() + i * row_stride, hidden.data() + src * row_stride, row_stride * sizeof(float));
    }
    return out;
}

std::pair<std::vector<float>, std::vector<float>> computePatchRoPE(
...
std::vector<int64_t> computeCuWindowSeqlens(
    ...
    (void)pH;
    (void)pW;  // silence -Wunused when NDEBUG
    return cu;
}

std::vector<float> buildBlockAttentionMask(
    size_t N, const std::vector<int64_t>& boundaries, float allowed, float blocked) {
    if (boundaries.empty() || boundaries.front() != 0 || static_cast<size_t>(boundaries.back()) != N) {
        throw std::runtime_error("buildBlockAttentionMask: boundaries must start at 0 and end at N");
    }
    std::vector<int> row_block(N);
    for (size_t b = 0; b + 1 < boundaries.size(); ++b)
        for (int64_t i = boundaries[b]; i < boundaries[b + 1]; ++i) row_block[i] = static_cast<int>(b);
