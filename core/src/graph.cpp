...
static size_t tensorByteSize(const Qnn_Tensor_t* t) {
    ...
    if (it == k_dtype_size.end()) throw std::runtime_error("tensorByteSize: unsupported dtype");
    return n * it->second;
}
...
template <typename T>
static void tfNToFloat(float* out, const T* in, int32_t offset, float scale, size_t n) {
