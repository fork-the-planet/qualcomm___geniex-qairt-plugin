    ...
    : half_dim_(head_dim / 2)
{
    ...
    for (size_t i = 0; i < half_dim_; ++i) {
        inv_freq_[i] = 1.f / std::pow(theta, static_cast<float>(i * 2) /
                                              static_cast<float>(head_dim));
    }
}

std::pair<std::vector<float>, std::vector<float>>
RotaryEmbedding::forward(const std::vector<int32_t>& position_ids) const {
...
LongRoPEEmbedding::forward(const std::vector<int32_t>& position_ids) const {
    ...
    if (scale_ratio <= 1.f) {
        ...
        scaling_factor = std::sqrt(1.f + std::log(scale_ratio) /
...
std::pair<std::vector<float>, std::vector<float>>
PartialRoPEEmbedding::forward(const std::vector<int32_t>& position_ids) const {
    const size_t n = position_ids.size();
...
std::vector<int32_t> get_position_ids(size_t n_past, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i)
        ids[i] = static_cast<int32_t>(n_past + i);
    return ids;
}
...
std::vector<float> get_attention_mask(size_t n_past,
                                      size_t curr_len,
                                      size_t seq_len,
                                      size_t kv_len) {
...
std::vector<float> tokensToEmbedding(const std::vector<int32_t>& token_ids,
                                     const float*                 embedded_tokens,
                                     size_t                       hidden_size) {
    const size_t n = token_ids.size();
    std::vector<float> out(n * hidden_size);
    for (size_t i = 0; i < n; ++i) {
        const float* src = embedded_tokens +
                           static_cast<size_t>(token_ids[i]) * hidden_size;
