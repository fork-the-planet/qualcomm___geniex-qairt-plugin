...
std::pair<xt::xarray<float>, xt::xarray<float>> RotaryEmbedding::forward(
    const xt::xarray<int32_t>& position_ids) const {
...
XTensorLLMUtils::XTensorLLMUtils(size_t seq_len, size_t kv_len, size_t hidden_size, size_t /*num_heads*/,
    size_t num_kv_heads, size_t head_dim, size_t start_layer_idx, size_t end_layer_idx, size_t vocab_size,
    const std::string& in_states_name, const std::string& out_states_name, float rope_theta, int32_t eos_token_id) {
    seq_len_         = seq_len;
    kv_len_          = kv_len;
...
xt::xarray<float> XTensorLLMUtils::tokensToEmbedding(
    const std::vector<int32_t>& token_ids, const xt::xarray<float>& embedding_table) const {
    xt::xarray<float> result = xt::zeros<float>(std::vector<std::size_t>{1, seq_len_, hidden_size_});

    std::size_t n = std::min(token_ids.size(), seq_len_);
    for (std::size_t i = 0; i < n; ++i) {
        xt::view(result, 0, i, xt::all()) = xt::view(embedding_table, token_ids[i], xt::all());
    }
    return result;
}

xt::xarray<float> XTensorLLMUtils::tokenToEmbedding(int32_t token_id, const xt::xarray<float>& embedding_table) const {
    xt::xarray<float> result = xt::zeros<float>(std::vector<std::size_t>{1, static_cast<std::size_t>(1), hidden_size_});

    xt::view(result, 0, 0, xt::all()) = xt::view(embedding_table, token_id, xt::all());
    return result;
}

xt::xarray<int32_t> XTensorLLMUtils::get_position_ids(std::size_t n_past, std::size_t curr_len) const {
    return xt::arange<int32_t>(static_cast<int32_t>(n_past), static_cast<int32_t>(n_past + curr_len));
}

std::pair<xt::xarray<float>, xt::xarray<float>> XTensorLLMUtils::get_cos_sin(
    const xt::xarray<int32_t>& position_ids) const {
    auto pos2d  = xt::reshape_view(position_ids, std::array<std::size_t, 2>{1, position_ids.shape()[0]});
    auto [c, s] = rope_.forward(pos2d);
    return {xt::expand_dims(c, 0), xt::expand_dims(s, 0)};
}

xt::xarray<float> XTensorLLMUtils::get_attention_mask(std::size_t n_past, std::size_t curr_len) const {
    xt::xarray<float> mask = xt::ones<float>(std::array<std::size_t, 2>{seq_len_, total_len_}) * (-1000000000.f);

    if (curr_len == 0) return xt::expand_dims(xt::expand_dims(mask, 0), 0);

    if (n_past > 0) {
        xt::view(mask, xt::range<std::size_t>(0, curr_len), xt::range<std::size_t>(0, n_past)) = 0.f;
    }

    xt::xarray<int> rows =
        xt::reshape_view(xt::arange<int>(0, static_cast<int>(curr_len)), std::array<std::size_t, 2>{curr_len, 1});
    ...
    xt::view(mask, xt::range<std::size_t>(0, curr_len), xt::range<std::size_t>(kv_len_, kv_len_ + curr_len)) = causal;
...
std::map<std::string, std::vector<float>> XTensorLLMUtils::prepare_inputs(
    const xt::xarray<float>& input_embeds, std::size_t n_past, std::size_t curr_len) const {
...
void XTensorLLMUtils::update_kv_cache(std::vector<std::vector<float>>& input_data,
    std::vector<std::vector<float>>& output_data, int n_past, int curr_len, size_t out_seq_len) {
    for (std::size_t i = start_layer_idx_; i <= end_layer_idx_; ++i) {
        ...
        auto ki = xt::adapt(
            input_data[input_tensor_order[key_in]], std::vector<std::size_t>{num_kv_heads_, 1, head_dim_, kv_len_});
        auto ko = xt::adapt(output_data[output_tensor_order[key_out]],
            std::vector<std::size_t>{num_kv_heads_, 1, head_dim_, out_seq_len});
        xt::view(ki, xt::all(), xt::all(), xt::all(), xt::range(n_past, n_past + curr_len)) =
            xt::view(ko, xt::all(), xt::all(), xt::all(), xt::range(0, curr_len));

        auto vi = xt::adapt(
            input_data[input_tensor_order[val_in]], std::vector<std::size_t>{num_kv_heads_, 1, kv_len_, head_dim_});
        auto vo = xt::adapt(output_data[output_tensor_order[val_out]],
            std::vector<std::size_t>{num_kv_heads_, 1, out_seq_len, head_dim_});
        xt::view(vi, xt::all(), xt::all(), xt::range(n_past, n_past + curr_len), xt::all()) =
            xt::view(vo, xt::all(), xt::all(), xt::range(0, curr_len), xt::all());

        input_data[input_tensor_order[key_in]] = std::vector<float>(ki.begin(), ki.end());
        input_data[input_tensor_order[val_in]] = std::vector<float>(vi.begin(), vi.end());
    }
}

void XTensorLLMUtils::transfer_kv_cache(
    std::vector<std::vector<float>>& dst, std::vector<std::vector<float>>& src, int n_past) {
    for (std::size_t i = start_layer_idx_; i <= end_layer_idx_; ++i) {
        ...
        if (input_tensor_order.count(key_name)) {
            ...
            auto sk = xt::adapt(src[idx], std::vector<std::size_t>{num_kv_heads_, 1, head_dim_, src_kv});
            auto dk = xt::adapt(dst[idx], std::vector<std::size_t>{num_kv_heads_, 1, head_dim_, dst_kv});
            xt::view(dk, xt::all(), xt::all(), xt::all(), xt::range(0, n_past)) =
                xt::view(sk, xt::all(), xt::all(), xt::all(), xt::range(0, n_past));
        ...
        if (input_tensor_order.count(val_name)) {
            ...
            auto sv = xt::adapt(src[idx], std::vector<std::size_t>{num_kv_heads_, 1, src_kv, head_dim_});
            auto dv = xt::adapt(dst[idx], std::vector<std::size_t>{num_kv_heads_, 1, dst_kv, head_dim_});
            xt::view(dv, xt::all(), xt::all(), xt::range(0, n_past), xt::all()) =
                xt::view(sv, xt::all(), xt::all(), xt::range(0, n_past), xt::all());
            dst[idx] = std::vector<float>(dv.begin(), dv.end());
        }
    }
}

void XTensorLLMUtils::writeInputs(Graph& graph, const std::vector<std::vector<float>>& input_data) const {
...
void XTensorLLMUtils::readOutputs(const Graph& graph, std::vector<std::vector<float>>& output_data) const {
